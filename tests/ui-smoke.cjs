...
(async()=>{
 ...
 try{
  ...
  assert.deepEqual(await page.evaluate(()=>({phase:Game.p.run.phase,build:Game.p.run.build,running:Game.running,paused:Game.paused})),{phase:'battle',build:'counter',running:true,paused:false});
  ...
  await page.evaluate(()=>{Game.p.run.phase='rest';Game.p.run.restUsed=false;Game.openRunUI();});
  ...
  console.log('PASS fresh start, build selection, settings, camp continuation; no browser errors');
