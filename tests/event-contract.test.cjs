...
function fixture(job = 'warrior') {
  ...
  const G = {
    ...
    grantRelic(id) {
      grants.push(id);
      G.p.run.relics.push(id);
    },
    openRunUI() {},
    runRandom(seed) {
      calls.push(seed);
      return { 99: 0.2, 101: 0.4, 103: 0.8 }[seed];
    }
  };
  const ctx = vm.createContext({ window: { Game: G } });
  for (const file of ['event-data.js', 'events.js', 'event-validation.js']) {
    ...
    state: () =>
      JSON.parse(
        JSON.stringify({
          gold: G.p.gold,
          potions: G.p.potions,
          materials: G.p.materials,
          mp: G.p.mp,
          hp: G.p.hp,
          buffs: G.p.run.buffs,
          relics: G.p.run.relics,
          claims: G.p.run.events.length,
          saves,
          grants
        })
      )
...
test('all pre-migration event choices retain resources effects and single-claim behavior', () => {
  ...
  const rows = JSON.parse(
    fs.readFileSync(path.join(__dirname, 'fixtures/event-baseline.json'), 'utf8')
  );
  for (const row of rows) {
    const f = fixture(row.job);
    f.G.p.run.event = { id: row.event, roll: row.roll, relic: 'alpha', result: null, choice: null };
    // When: the choice is submitted twice.
    const first = f.G.chooseEventOption(row.choice),
      second = f.G.chooseEventOption(row.choice);
    // Then: costs, rewards and one-time claim match the original implementation.
    assert.deepEqual(
      { first, second, after: f.state() },
      { first: row.first, second: row.second, after: row.after },
      JSON.stringify({ event: row.event, choice: row.choice, job: row.job, roll: row.roll })
    );
  }
});
test('event preparation retains seeded sampling order and selected reward', () => {
  ...
  assert.deepEqual(JSON.parse(JSON.stringify(G.p.run.event)), {
    id: 'smith',
    roll: 0.4,
    relic: 'beta',
    result: null,
...
test('insufficient event resources leave state unchanged', () => {
  for (const [event, choice, resource] of [
    ['spring', 'drink', 'gold'],
    ['archive', 'study', 'mp'],
    ['smith', 'temper', 'materials'],
    ['fox', 'feed', 'potions']
  ]) {
    const f = fixture();
    f.G.p[resource] = 0;
    f.G.p.run.event = { id: event, roll: 0, relic: null, result: null, choice: null };
...
test('persisted event parser retains accepted legacy falsy relic values and extra fields', () => {
  const { G } = fixture();
  for (const relic of [null, undefined, '', 0, false, 'alpha']) {
    const raw = { id: 'spring', roll: 0.5, relic, result: null, choice: null, extra: 'preserved' };
...
test('persisted event parser rejects invalid fields and incoherent outcomes', () => {
  const { G } = fixture(),
    base = { id: 'spring', roll: 0.5, relic: null, result: null, choice: null };
  const invalid = [
    undefined,
    [],
    {},
  ...
  for (const raw of invalid)
    assert.throws(() => G.parseEventSnapshot(raw), { name: 'EventSnapshotError' });
});
