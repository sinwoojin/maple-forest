...
const root=path.resolve(__dirname,'..'),dir=fs.mkdtempSync(path.join(os.tmpdir(),'maple-type-contract-'));
const source=path.join(dir,'contract.ts'),config=path.join(dir,'tsconfig.json');
...
try{
 fs.writeFileSync(config,JSON.stringify({compilerOptions:{...options,noEmit:true,rootDir:dir},files:[path.join(root,'types/events.d.ts'),source]}));
 const check=content=>{fs.writeFileSync(source,content);return spawnSync(process.execPath,[compiler,'-p',config,'--pretty','false'],{encoding:'utf8'});};
 // Given: the exact production contracts, with a valid control case.
 const valid=check("const cost: MapleForest.EventCost={gold:20,job:'mage'}; const effect: MapleForest.Effect={chance:.5,win:{relic:true},lose:{hurt:.2}}; const phase: MapleForest.RunPhase='event';");
 ...
 const cases=[
  ...
  ['ambiguous chance and direct reward',"const effect: MapleForest.Effect={chance:.5,win:{},lose:{},gold:10};"],
  ['unparsed external data',"declare const raw: unknown; const saved: MapleForest.EventSnapshot=raw;"],
  ['unknown host capability',"declare const host: MapleForest.LegacyEventHost; host.getHealth();"]
 ];
 for(const [name,content]of cases){
  ...
  assert.equal(result.stderr,'');console.log('PASS rejects '+name);
 }
 console.log('PASS valid control and 8 rejected type-contract cases');
}finally{for(const file of [source,config])if(fs.existsSync(file))fs.unlinkSync(file);fs.rmdirSync(dir);}
