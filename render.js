...
(() => {
  ...
  function resize(){const box=canvas.getBoundingClientRect();G.width=Math.round(720*box.width/box.height);G.height=720;canvas.width=G.width;canvas.height=720;ctx.imageSmoothingEnabled=false;}
  new ResizeObserver(resize).observe(canvas);resize();
  function label(x,y,text,color='#fff9e9',size=14){ctx.font=`bold ${size}px "Malgun Gothic",sans-serif`;ctx.textAlign='center';ctx.lineWidth=3;ctx.strokeStyle='#352a24';ctx.strokeText(text,x,y);ctx.fillStyle=color;ctx.fillText(text,x,y);}
  function drawEnemy(e,t){
    ...
    if(boss){const name=G.p.run?.active?G.runInfo().name+' 수호자':e.zone==='cavern'?'버섯 군주':'숲의 수호자';label(e.x,e.y-126,G.bossAwake(e)?name:'잠든 '+name,'#f7b749',16);
      if(e.phase>1.6&&e.phase<2.5&&G.bossAwake(e)){ctx.fillStyle='#cf504b66';ctx.fillRect(e.x-170,e.y-8,340,8);label(e.x,e.y-152,'충격파! 점프로 피하세요','#fff9e9',14);}
    }
  ...
  function draw(){
    const p=G.p,cave=p.zone==='cavern',t=G.time,run=p.run?.active,info=run?G.runInfo():null;G.camera=Math.max(0,Math.min(G.worldWidth-G.width,p.x-G.width*.35));
    ...
    if(run)Art.stageForeground(ctx,G.camera,t,G.width,G.height,info);else (cave?Art.cavernForeground:Art.foreground)(ctx,G.camera,t,G.width,G.height);
    for(const e of G.effects){ctx.globalAlpha=Math.min(1,e.life*3);label(e.x,e.y,e.text,e.color,e.text.includes('LEVEL')?32:24);}ctx.globalAlpha=1;ctx.restore();
  }
