...
for (const name of files)
  if (!/^[a-zA-Z0-9-]+\.(html|css|js)$/.test(name) || !fs.statSync(path.join(root, name)).isFile())
    throw Error('Invalid public asset: ' + name);
fs.mkdirSync(output, { recursive: true });
for (const entry of fs.readdirSync(output)) {
  if (!fs.statSync(path.join(output, entry)).isFile())
    throw Error('Unexpected directory in public output');
