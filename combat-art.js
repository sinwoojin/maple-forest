...
(() => {
 ...
 A.danger=(c,h)=>{if(!Number.isFinite(h.x)||!Number.isFinite(h.y))return;c.save();const warning=h.warning||h.life>0&&h.label;c.strokeStyle=warning?'#f7b749':'#cf504b';c.fillStyle=warning?'#f7b74933':'#cf504b66';c.lineWidth=3;c.setLineDash(warning?[8,5]:[]);c.beginPath();if(h.width)c.rect(h.x-h.width/2,h.y-(h.height||12)/2,h.width,h.height||12);else c.arc(h.x,h.y,h.radius||12,0,Math.PI*2);c.fill();c.stroke();c.setLineDash([]);if(h.label){const warningLabel=({root:'뿌리 솟구침',charge:'돌진',summon:'소환',crystal:'수정 낙하',shockwave:'충격파 · 점프',ice:'빙결',blizzard:'눈보라',leap:'도약 강타',rift:'균열',darkbolt:'암흑 탄환',eclipse:'월식 · 점프',arrow:'화살',bash:'방패 강타',bomb:'폭발',pulse:'파동',ambush:'기습'})[h.label]||h.label;c.font=`bold ${12*(G.textScale||1)}px "Malgun Gothic",sans-serif`;c.textAlign='center';c.strokeStyle='#352a24';c.lineWidth=4;c.strokeText(warningLabel,h.x,h.y-(h.radius||h.height||20)-12);c.fillStyle='#fff9e9';c.fillText(warningLabel,h.x,h.y-(h.radius||h.height||20)-12);}c.restore();};
})();
