...
(() => {
  ...
  const allowed = () =>
    G.running && !G.paused && !modal().open && (!G.p.run?.active || G.p.run.phase === 'battle');
  const sync = () => {
    for (const action of ['left', 'right', 'attack']) {
      const held =
        [...pointers.values()].some(v => v.action === action) ||
        [...keyboard.values()].includes(action);
      if (held && allowed()) G.keys.add(action);
      else G.keys.delete(action);
    }
  };
  G.clearInput = () => {
    pointers.clear();
    keyboard.clear();
    G.keys.clear();
    document.querySelectorAll('[data-pressed]').forEach(b => b.removeAttribute('data-pressed'));
  };
  G.bindInputs = () => {
    const holds = {
      ArrowLeft: 'left',
    ...
    window.addEventListener('keydown', e => {
      ...
      if (e.code === 'KeyI' && !e.repeat && !modal().open) {
        e.preventDefault();
        G.UI.inventory();
        return;
      }
      ...
      if (holds[e.code] && allowed() && (!e.repeat || keyboard.has(e.code))) {
        keyboard.set(e.code, holds[e.code]);
        sync();
      }
    ...
    for (const b of document.querySelectorAll('[data-action],[data-hold]')) {
      const action = b.dataset.hold || b.dataset.action,
        hold = !!b.dataset.hold || action === 'attack';
      b.addEventListener('pointerdown', e => {
        ...
        if (hold) sync();
        if (!b.dataset.hold) G.act(action);
      });
      const release = e => {
        pointers.delete(e.pointerId);
        if (![...pointers.values()].some(v => v.button === b)) b.removeAttribute('data-pressed');
        sync();
      };
      for (const type of ['pointerup', 'pointercancel', 'lostpointercapture'])
        b.addEventListener(type, release);
      b.addEventListener('click', e => {
        if (e.detail === 0 && !b.disabled) G.act(action);
      });
