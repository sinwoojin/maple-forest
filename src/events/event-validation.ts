...
(() => {
  ...
  class EventSnapshotError extends Error {
    constructor(message: string) { super(message); this.name = 'EventSnapshotError'; }
  }
  ...
  const validSnapshot = (raw: unknown): raw is MapleForest.EventSnapshot => {
    ...
    if (relic !== undefined && relic !== null && relic !== false && relic !== 0 && relic !== '' &&
      (typeof relic !== 'string' || !Object.hasOwn(G.relicCatalog || {}, relic))) return false;
    if (result !== null && typeof result !== 'string') return false;
    if (choice !== null && choice !== 'leave' && !data.choices.some(option => option.id === choice)) return false;
    return true;
  };
  ...
  G.parseEventSnapshot = raw => {
    ...
    if ((raw.result === null) !== (raw.choice === null)) throw new EventSnapshotError('Invalid event result');
    return raw;
  };
})();
